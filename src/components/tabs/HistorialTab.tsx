...
export default function HistorialTab({ historial, historialStatus, cargarHistorialReal, loggedIn }: HistorialTabProps) {
  useEffect(() => {
    ...
    <div className="flex flex-col gap-5">
      <div className="rounded-2xl bg-white/[0.03] ring-1 ring-white/5 p-5">
        <div className="flex items-center justify-between mb-1">
          <h3 className="text-sm font-medium text-white">Historial real (todas tus sesiones)</h3>
      ...
      {hasData && (
        <>
            ...
            <ChartCanvas
              config={{
                ...
                data: {
                  ...
                  datasets: [
                    { label: 'Humedad %', data: historial!.map((r) => r.humedad_suelo_pct), borderColor: '#268a4a', backgroundColor: 'rgba(38,138,74,0.08)', tension: 0.3, pointRadius: 2, fill: true },
                    { label: 'NDVI×100', data: historial!.map((r) => (r.ndvi || 0) * 100), borderColor: '#2d6e8f', backgroundColor: 'rgba(45,110,143,0.08)', tension: 0.3, pointRadius: 2, fill: true },
                  ],
                },
          ...
          <div className="rounded-2xl bg-white/[0.03] ring-1 ring-white/5 p-5">
            <h3 className="text-xs text-white/40 uppercase tracking-wide mb-3">Últimas 15 lecturas</h3>
            <div className="flex flex-col gap-2 max-h-80 overflow-y-auto">
              {historial!
                .slice(-15)
                .reverse()
                .map((r, i) => (
                  <div key={i} className="grid grid-cols-[110px_80px_1fr] gap-2 text-xs py-2 border-b border-white/5 last:border-0">
