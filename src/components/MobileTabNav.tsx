...
export default function MobileTabNav({ active, onSelect, labels }: MobileTabNavProps) {
    ...
    <div className="lg:hidden relative border-b border-white/5 bg-[#0d0d0f]">
      ...
      >
        <span className="flex items-center gap-2">
          <span className="text-[10px] uppercase tracking-wide text-white/40">Sección</span>
          <ActiveIcon size={14} />
          {labels[active]}
        </span>
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full z-30 bg-[#141416] ring-1 ring-white/10 max-h-80 overflow-y-auto">
          {TABS.map((tab) => {
            const Icon = tab.icon;
            return (
              <button
